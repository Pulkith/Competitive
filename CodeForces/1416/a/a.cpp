...
void solve() {
    ...
    vi A(N);
    int minn = INF;
    multiset<int> left, right, center;
    FOR(i, 0, N ){
        cin >> A[i];
        chmin(minn, A[i]);
        // if(i == 0) left.insert(A[i]);
        // else if(i == N-1) right.insert(A[i]);
        // else 
        center.insert(A[i]);
    }
    vi ans(N, -1);
    ans[N-1] = minn;
    for(int l = 1, r = N - 2, cnt = N - 2; l <= r; ++l, --r, --cnt ) {
            center.erase(center.equal_range(A[l-1]).f);
            center.erase(center.equal_range(A[r+1]).f);
            ans[cnt] = *center.begin();
    }
