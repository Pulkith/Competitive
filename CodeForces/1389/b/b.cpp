...
void solve() {
    ...
    vt<int> A(N), B(N);
    FOR(i, 0, N) cin >> A[i];
    FOR(i, 0, N) B[i] = A[i] + (i != 0 ? B[i-1] : 0);
    ll sum = 0;
    ll maxx = sum = A[0];

    FORE(i, 1, K) {
        ll cur = sum += A[i];
        int cur_left = 0, cur_total = 0;
        int move = 1;
        
        while(cur_left < Z && cur_total + i < K) {
            if(move == 1) ++cur_left;
            ++cur_total;
            cur += A[i-move];
            move = !move;
        ...
        int extra = K - (cur_total + i );
        ll save = cur;
        if(move == 1 && extra > 0) cur += B[i+extra] - B[i];
        else if(move == 0 && extra > 0) cur += B[i+extra-1] - B[i-1];
