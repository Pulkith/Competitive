...
int test_case([[maybe_unused]] int test_case = 0) {
    ...
    vt<int> node1 = query(1);
    FOR(i, 0, N)
        if(node1[i] == 1)
            edges.pb({i+1, 1});

    vt<int> odd_dist, even_dist;

    FOR(i, 0, N) {
        if(node1[i] == 0) continue;
        if(node1[i]&1)
            odd_dist.pb(i+1);
        else
            even_dist.pb(i+1);
    ...
    vt<int> nodes = (sz(odd_dist) < sz(even_dist)) ? odd_dist : even_dist;
    ...
    set<pr<int, int>> unique;

    for(auto edge : edges)
        unique.insert({min(edge.f, edge.s), max(edge.f, edge.s)});
    cout << "!" << '\n';
