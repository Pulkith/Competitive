/**
 * author: DespicableMonkey
 * created: 06.16.2021 00:15:02
 * Potatoes FTW!
 **/ 

#include<bits/stdc++.h>
#if LOCAL
    #include <DespicableMonkey/Execution_Time.h>
    #include <DespicableMonkey/Debug.h>
#endif

using namespace std;

typedef int64_t ll;

#define pb push_back
#define f first
#define s second
#define x first
#define y second

#define my_brain_hurts int T = 1;

#define all(c) (c).begin(), (c).end()
#define sz(x) (int)(x).size()
#define ts(x) to_string(x)

#define FOR(i,a,b) for (int i = (a); i < (b); ++i)
#define FORE(i, a, b) for(int i = (a); i<= (b); ++i)

template<typename T, typename U> using pr = pair<T, U>;
template<typename T> using vt = vector<T>;
template<class T, class U> T cdiv(T a, U b) { return a/b+((a^b)>0&&a%b); } // divide a by b rounded up
template<class T> bool cmin(T& a, const T& b) { return b < a ? a = b, 1 : 0; }
template<class T> bool cmax(T& a, const T& b) { return a < b ? a = b, 1 : 0; }

[[maybe_unused]] const int MOD = 1'000'000'007;
[[maybe_unused]] const double EPS = (1e-9);
[[maybe_unused]] const int INF = 2'000'000'000; //0xc0, 0x3f. Pos, Neg Inf for memset. Comparison = 0x3f3f3f3f
[[maybe_unused]] const int dx[4] = {1, 0, -1, 0}, dy[4] = {0, 1, 0, -1}; //DRUL

inline namespace CP {
    inline namespace IO {
        void setIn(string s)  { (void)!freopen(s.c_str(),"r",stdin); }
        void setOut(string s) { (void)!freopen(s.c_str(),"w",stdout); }
        void FastIO() {
            cin.tie(nullptr)->sync_with_stdio(0);
            std::cout << std::fixed << std::showpoint;
            std::cout << std::setprecision(14);
        }
        void SetIO(string s = "", string t = "") {
            cin.exceptions(cin.failbit); // throws exception when do smth illegal ex. try to read letter into int
            if(sz(t)) setIn(s), setOut(t);
            else if (sz(s)) setIn(s+".in"), setOut(s+".out"); // for old USACO
        }
    }
    inline namespace Output {
        bool debug = 0;
        string operator+(string str, int num){return str + ts(num);}
        string operator+(int num, string str) { return str + ts(num); }
        string to_string(const char* s) { return string(s);}
        #if !defined LOCAL
            #define dbg(...)
        #endif
        template<class T> void outv(vector<T> v, int add = 0, bool standard = 1) {for(T& i : v) (standard?cout:cerr) << (i+add) << " "; cout << '\n'; }
        template<class T> void put(T s) { cout << s << '\n'; }
        #define putr(s) return put(s), 0
    }
}
/*|||||||||||||||||| ||||||||||||||||||  CODE STARTS HERE  |||||||||||||||||| |||||||||||||||||| */

const int MX = (4e5+5); //Check the limits idiot
int N, M;
int a[MX];
string grid[MX];
map<pr<int, int>, vt<pr<int, int>>> movable_nodes;
map<pr<int, int>, int> IDs;
map<int, pr<int, int>> rev_IDS;
int indexes[MX];
int cur_index = 1;
int low[MX];
vt<int> cur_nodes;
int vis[MX];
vt<vt<int>> SCCs;
int which_SCC[MX];
void Tarjans(int loc, int i, int j) {
    indexes[loc] = cur_index;
    low[loc] = cur_index;
    ++cur_index;
    vis[IDs[{i, j}]] = 1;
    cur_nodes.pb(loc);
    for(auto near : movable_nodes[{i, j}]) {
        if(!indexes[IDs[{near.f, near.s}]]) {
            Tarjans(IDs[{near.f, near.s}], near.f, near.s);
            cmin(low[loc], low[IDs[{near.f, near.s}]]);

        }   
        else if(vis[IDs[{near.f, near.s}]]) {
            cmin(low[loc], indexes[IDs[{near.f, near.s}]]);
        }
    }

    if(indexes[loc] == low[loc]) {
        int cur_SCC = sz(SCCs);
        SCCs.pb({});
        while(true) {
            int cur = cur_nodes.back();
            SCCs.back().pb(cur);
            cur_nodes.pop_back();
            vis[cur] = 0;
            which_SCC[cur] = cur_SCC;
            if(cur == loc) break;

        }
    }

}

int test_case([[maybe_unused]] int test_case = 0) {
    cin >> N >> M;
    FOR(i, 0, N) cin >> grid[i];
    FOR(i, 0, N) cin >> a[i];


    FOR(i, 0, N) {
        FOR(j, 0, M) {
            if(i > 0 && grid[i-1][j] == '#') movable_nodes[{i, j}].pb({i-1, j});
            FOR(k, i+1, N) {
                if(grid[k][j] == '#') {
                    movable_nodes[{i, j}].pb({i-1, j});
                    break;
                }
            }
            bool found_left_node = false, found_right_node = false;
            for(int k = i; k < N && (!found_left_node||!found_right_node); ++k) {
                if(k != i && grid[k][j] == '#') break;
                if(!found_left_node && j>0 && grid[k][j-1] == '#') {
                    found_left_node = true;
                    movable_nodes[{i, j}].pb({k, j-1});
                }
                if(!found_right_node && j<M-1 && grid[k][j+1] == '#') {
                    found_right_node = 1;
                    movable_nodes[{i, j}].pb({k, j+1});
                }
            }
        }
    }
    int ind = 0;
    FOR(i, 0, N) {
        FOR(j, 0, M)
            if(grid[i][j] == '#')
                    IDs[{i, j}] = ind, rev_IDS[ind++] = {i, j};
    }
    FOR(i, 0, N) {
        FOR(j, 0, M) {
            if(!indexes[IDs[{i, j}]])
                Tarjans(IDs[{i, j}], i, j);
        }
    }

    return 0;
}

int main () {
    CP::IO::SetIO();
    CP::IO::FastIO();

    #if LOCAL
        //CP::IO::setIn("in1.txt");
        CP::ExecTime::use_clock();
        debug = true;
    #endif
    my_brain_hurts
    //cin >> T;
    for(int tt = 1; tt <= T; ++tt){
        //cout << "Case #" << tt << ": ";
        if (debug) { cerr << "\033[33m" << "\n[Test #" << (tt) << "]\n" << "\033[0m"; }
        test_case(tt);
    }

    #if LOCAL
        CP::ExecTime::log_time(0);
    #endif

    return 0;
}