...
void solve() {
    ...
    while(!empty(zeros) || !empty(ones)) {
        ++cur_index;
        bool last = (sz(ones) && sz(zeros) && *ones.begin() < *zeros.begin() || 
            sz(ones) && !sz(zeros));
        int index = -1;
        while(sz(last?ones:zeros) ) {

            auto x = (last?ones:zeros).lower_bound(index);
            if(x == (last?ones:zeros).end()) break;
            if(*x < index) break;

            ans[*x] = cur_index;
            index = *x;
            (last?ones:zeros).erase(x);
            last = !last;
            dbg(last, sz(ones), sz(zeros), cur_index, index);
