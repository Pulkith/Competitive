...
const int MOD =   1  , INF = 2 * MOD; //0xc0, 0x3f. Pos, Neg Inf for memset. Comparison = 0x3f3f3f3f
...
int N, K;
map<int, pr<int, int>> mp;
map<int, pr<int, int>> delta;
        ...
        if(y.s > 0) {
            ...
            if(choose+make >= K) {
                if(choose < K) {
                    ...
                    //we choose i prev elements, and K - i new elements
                    for(int i = max(0, K - make); i < K; ++i) {
                        //ans += (K-i)C(make)
                    }
