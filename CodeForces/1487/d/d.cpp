...
ll isPerfect(int b, int c) {
    ll ans = c * 1LL * c -  b * 1LL * b ;
    ll fo = floor(sqrt(ans));
    if(fo * fo == ans) return fo;
    return 0;
}
...
void solve() {
    int N; cin >> N;
    if(N < 5) cout << 0 nl
    else if(N <= 12) cout << 1 nl
    else if(N <= 24) cout << 2 nl
    else if(N == 25) cout << 3 nl
    else {
        int cnt = 3;
        ll add = 16;
        ll sum = 25;
