...
/*
    Find Shortest path between 3 points;
*/

void calc(pii mid, pii to) {
    for(auto i = to.f; i != mid.f; i += (to.f < mid.f ? 1 : -1))
        ans.insert({i, to.s});
    for(auto i = to.s;i != mid.s; i += (to.s < mid.s ? 1 : -1))
        ans.insert({mid.f, i});
}
