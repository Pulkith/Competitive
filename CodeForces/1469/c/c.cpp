...
void solve() {
    ...
    dp[0][1] = a[0] + K ;
    FOR(i,1, N) {
        dp[i][0] = max(dp[i-1][0] + 1 - K, a[i]);
        dp[i][1] = min(a[i] + 2*K - 1, dp[i-1][1] - 1 + K);
        if(dp[i][1] <= dp[i-1][0] || dp[i][0] >= dp[i-1][1]) {
            put("NO");
            rtn;
        }
    }   
    if(dp[0][0] != a[0] || dp[N-1][0] != a[N-1]) {
        put("NO");
        rtn;
    }
    put("YES");
