...
// the point that minimizes the sum of differeences in a set is the sum

//the median of an even set when calcualting absolute differences, 
//can be the ceil(n/2)th number
...
void solve() {
    ...
    while(top <= bottom) {
        ...
        while(l <= r) {
            vll nums;
            nums.pb(a[top][l]);
            if(top != bottom) nums.pb(a[bottom][l]);
            if(l != r) nums.pb(a[top][r]);
            if(l != r && top != bottom) nums.pb(a[bottom][r]);

            ll add = 0;

            sortt(nums);
            FR(i, sz(nums))
                add += abs(nums[i] - nums[sz(nums) / 2]);
            ans +=  add;
            ++l; --r;
        }
        ++top; --bottom;
