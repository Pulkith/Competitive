...
//Reverse Every substring of size K from [1...N-K+1]. O(N) math method, not as slow as O(N^2) naive reverse method
string calc(int K) {
    string first = s.substr(K-1);
...
int test_case([[maybe_unused]] int test_case = 0) {
    ...
    auto minn = min_element(s.begin(), s.end());

    if(count(s.begin(), s.end(), *minn) == 1) {
        int K = (minn - s.begin()) + 1;
        put(calc(K));
        putr(K);
    } else {
        string ans = "";
