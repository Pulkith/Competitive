...
struct Tree {
    ...
    void update(Node* cur){
        ...
        int left = (cur->left == nullptr) ? 1 : cur->left->data;
        int right = (cur->right == nullptr) ? 1 : cur->right->data;

        if(cur->val == '0') cur->data = left;
        else if(cur->val == '1') cur->data = right;
        else cur->data = left + right;
...
void solve() {
    ...
    FOR(i, 0, (1<<K)-1) {
        Node* node = new Node(s[i], 1, i&1);
        tree.add(i + 1, *node);
    }
    for(int i = tree.total; i >= 1; --i) {
        Node *cur = tree.query(i);

        if(cur->left == nullptr || cur->right == nullptr){
            cur->data = !(cur->val == '1' || cur->val == '0') + 1;
        }
        else {
            int left = cur->left->data;
            int right =  cur->right->data;
