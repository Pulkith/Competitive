...
ll solve2(int N, string s) {
    int c = calc(s);
    auto val = [&](int v) -> ll {
        return (1LL * (v * (v+1)))/2 - v;
    };
    ll ans = 0;
    ans += val(c);
        FOR(i, c, N-1) {
...
void solve() {
    ...
    ans += cur;
    put(ans + N);
    // int score1 = solve2(N, s);
    // reverse(s.begin(), s.end());
    // int score2 = solve2(N, s);
    // bool ok = 1;
    // calc(s);

    // dbg(score1, score2);
    // FOR(i, c, N) {
    //     if(s[i] == '?') s[i] = (s[i-1] == '1' ? '0' : '1');
    //     else {
    //         if(i > 0 && s[i] == s[i-1]) {
    //             ok = 0;
    //             break;
