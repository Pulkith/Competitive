...
void solve() {
    ...
    int comp = 0;
    int ind[6]{0};
    //Find maximum number of subeqeunces(stored in comp) of 4,8,15,16,23,42 that can be found in an array
    for(; ind[0] < sz(a[0]); ++ind[0]){
        FOR(j, 1, 6) {
            while(ind[j] < sz(a[j]) && a[j][ind[j]] < a[j-1][ind[j-1]]) ++ind[j];
            if(ind[j] == sz(a[j])) {
                cout << (N-comp*6) nl
                rtn;
            }
