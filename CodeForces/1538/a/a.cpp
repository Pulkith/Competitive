...
void solve() {
    ...
    auto minn = min_element(all(a));
    
    int distance = maxx - a.begin()+1, distance2 = minn-a.begin()+1;
    if(distance2 < distance) swap(distance, distance2);
    put(min({distance2, N+1-distance, N+1-distance2 + distance}));
}
