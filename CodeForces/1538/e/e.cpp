...
pr<ll, string> comp(string x, string y) {
    pr<ll, string> ans = {0LL, ""};
    string fir = (sz(x) >= 3 ? x.substr(sz(x) - 3) : x);
    string sec = (sz(y) >= 3 ? y.substr(0, 3) : y);

    ans.f =  calc(fir + sec);
    ans.s = "";

    if(sz(x) >= 6) ans.s = x.substr(0, 3) + ans.s;
    if(sz(y) >= 6) ans.y += y.substr(sz(y) - 3);
...
void solve() {
    int N; cin >> N;
    map<string, pr<ll, string>> cnt;
    string last;
    FOR(i, 0, N) {
        string v, change, ne;
        cin >> v >> change >> ne;
        if(change == ":=") {
            string shrink = (sz(v) <= 6 ? v : v.substr(0, 3) + v.substr(sz(v) - 3));
            cnt[v] = {calc(ne), shrink};
        } else {
            char dum; string ne2;
            cin >> dum >> ne2;
            assert(change == "=" && dum == '+');
            auto ans = comp(cnt[ne].s, cnt[ne2].s);
            cnt[v] = {cnt[ne].f + cnt[ne2].f + ans.f, ans.s};
        }
        last = v;
    }
    put(cnt[last].f);
