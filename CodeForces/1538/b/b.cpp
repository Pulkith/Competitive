...
void solve() {
    ...
    sort(all(a));
    vt<int> freq(10005, 0);
    vt<int> total_sum(10005, 0);
    FOR(i, 0, N) ++freq[a[i]], total_sum[a[i]] += a[i];
    partial_sum(freq.begin(), freq.end(), freq.begin());
    partial_sum(total_sum.begin(), total_sum.end(), total_sum.begin());
    ...
    //2 4 5 5 
    //2 6 11 16
    //need to check at current average, if everything after average, can equal everything before average
    int ans = INF;
    for(int i = 10004; i >= 1; --i) {
        int num_greater = freq.back() - freq[i-1];
        int num_less = freq[i-1];

        int extra = total_sum.back() - total_sum[i-1] - num_greater * i;
        int need = num_less * i - total_sum[i-1];

        dbg(i, num_greater, num_less, extra, need);

        if(need == extra) {
            cmin(ans, freq.back() - freq[i]);
        }
    }
    put(ans == INF ? -1 : ans);
}
