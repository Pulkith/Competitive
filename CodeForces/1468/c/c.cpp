...
void solve() {
    int Q;
    cin >> Q;
    multiset<pr<int, int>> mono;
    multiset<pr<int, int>, pred> poly;
    int cnt = 1;
    while(Q--) {
        int index; cin >> index;
        if(index == 1) {
            ...
            auto serve = *mono.begin();
            mono.erase(serve); poly.erase(serve);
            cout << serve.f << " ";
        }
        else if(index == 3) {
            auto serve = *poly.begin();
            mono.erase(serve); poly.erase(serve);
            cout << serve.f << " ";
        }
    }
