...
void solve() {
    string s;
    cin >> s;
    // z[i] has the maximum length in s.substring(i) that matches the prefix of s
    calculate(sz(s), s);
    int maxZ = 0;
    FOR(i, 0, sz(s)) {
        //if Z[i] == N-i, the suffix mathces the prefix, and if 
        //the maxZ we've seen so far is >= N-i, we've seen this string in the middle of the
        //string before
        if(z[i] == sz(s) - i && maxZ >= sz(s) - i) {
            put(s.substr(i));
            return;
        } 
        cmax(maxZ, z[i]);
