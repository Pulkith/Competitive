...
void solve() {
    ...
    int min_index = N - 1;
    while (min_index >= 0 && A[min_index] == min_index + 1) --min_index;

    double sorted_prob = (min_index == -1 ? 1 : 0);

    FOR(i, 0, M) {
        int range; double prob;
        cin >> range >> prob;
        --range;
        if (range >= min_index) {
            sorted_prob += ((1 - sorted_prob) * prob);
        }
    }
