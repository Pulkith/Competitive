...
void solve() {
    ...
    string prefix[5005], suffix[5005];
    FOR(i, 0, N)
        prefix[i] = (i == 0 ? "" : prefix[i - 1]) + s[i];
    ROF(i, N-1,0)
        suffix[i] = s[i] + (i == N-1 ? "" : suffix[i+1]);
    ...
    string ans = "";
    if(i >= 0 && j < N)
        ans = prefix[i] + suffix[N - 1 - j];

    FOR(k, i + 1, j) {
        if(isPal(prefix[k] + suffix[j]))
            ans = prefix[k] + suffix[j];
        else if(isPal(prefix[i] + suffix[j - (k - i)]))
            ans = prefix[i] + suffix[j - (k - i)];
    }
