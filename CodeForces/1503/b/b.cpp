...
void solve() {
    int N; cin >> N;
    int rem = N * N;
    int blocked_color; cin >> blocked_color;
    auto query = [&](int color, int x, int y, int rem) -> int {
        cout << color << " " << x << " " <<  y << '\n';
        ff();
    ...
    vt<vt<pr<int, int>>> colors(3, vt<pr<int, int>>());

    FOR(i, 0, N)
        FOR(j, 0, N)
            colors[(i+j)&1?2:1].pb({i+1, j+1});

    while(rem > 0) {
        int color, arr_index;
        if(blocked_color == 1) {
            if (sz(colors[2])) color = arr_index = 2;
            else color = 3, arr_index = 1;
        } else if(blocked_color == 2) {
            if (sz(colors[1])) color = arr_index = 1;
            else color = 3, arr_index = 2;
        } 
        else color = arr_index = (sz(colors[1]) ? 1 : 2);

        blocked_color = query(color, colors[arr_index].back().x, colors[arr_index].back().y, --rem);
        colors[arr_index].pop_back();
    }
