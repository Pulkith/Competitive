...
string decrease(string N, int index) {
    if(index == 0 && N[index] == '1') return N.substr(1);

    if(N[index] == '0') {
        N[index] = '9';
        N = decrease(N, index-1);
    } else {
        N[index] -= 1;
    }
    return N;
...
void solve() {
    ...
    FOR(i, 0, sz(N) - 1) {
        if(N[sz(N)-1 - i] == '9');
            ...
            N[sz(N)-1-i] = '9';
            N = decrease(N, sz(N) - 2 - i);
        }
        cmax(maxx, calculate(N));
    }
    put(maxx);
}
