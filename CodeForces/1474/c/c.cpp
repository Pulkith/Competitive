...
void read(int& elemen) {
    cin >> elemen;
}

#define re(var) int var, read(var);

bool solve(int x, multiset<int> ms) {
    while(sz(ms)) {
        int largest = *ms.rbegin();
        int dif = x - largest;
        if(ms.find(dif) == ms.end()) return 0;
        if(dif == largest && ms.count(dif) < 2) return 0;
        ms.erase(ms.lower_bound(dif));
        ms.erase(ms.lower_bound(largest));
        x = largest;
    }
    return 1;
}
...
void solve() {
    ...
    sort(all(trav));
    for(int i = 0; i < N - 1; ++i) {
        bool ok = solve(trav[i] + trav[N-1], ms);
        if(ok) {
            put("YES");
            put(trav[i] + trav[N-1]);
            int x = trav[i] + trav[N-1];
            while(sz(ms)) {
                int largest = *ms.rbegin();
                int dif = x - largest;
                cout << dif << " " << largest << '\n';
