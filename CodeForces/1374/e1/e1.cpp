...
struct pred { 
    bool operator()(const vt<int> &l, const vt<int> &r) { 
        return l[0] < r[0];
    } 
};
...
void solve() {
    ...
    partial_sum(both.begin(), both.end(), both.begin());
    ll best = INF * 1LL * 10;
    
    FOR(i, 0, sz(both) + 1) {  
        ll cur = (i > 0) ? both[i-1] : 0;
        int need = K - i;
        if(need < sz(A) && need < sz(B) && need >= 0) 
            cmin(best, cur + A[need] + B[need]);
    }
    put(best == INF* 1LL * 10 ? -1 : best);
}
