...
void add(int x, int delta) {
    if(mp.find(x) != mp.end())
        top.erase(top.lower_bound(mp[x]));
    mp[x] += delta;
    top.insert(mp[x]);
}
...
void solve() {
    ...
    while(Q--) {
        ...
        add(num, c == '-' ? -1 : 1);
        auto it = top.begin();
        if(sz(top) >=1 && *it >= 8) put("YES");
        else if(sz(top) >= 2 && *it >= 4) {
            int val = *it;
            int sec_val = *++it;
            if(*it >= 4) put("YES");
            else if(val >= 6 && *it >= 2) put("YES");
            else if(sz(top) >= 3 && sec_val >= 2 && *++it >= 2) put("YES");
            else put("NO");
        }
        else put("NO");
    }
