...
void solve() {
    ...
    FOR(i, 1, sz(ind)) {
        if(ind[i].f != ind[i].s && ind[i].f != N-1 && a[ind[i-1].s] < a[ind[i].f + 1]) {
            cmax(ans, (ind[i-1].s - ind[i-1].f + 1) + (ind[i].s - ind[i].f + 1) - 1);
        }
        if(ind[i-1].f != ind[i-1].s && ind[i-1].s != 0 && a[ind[i-1].s - 1] < a[ind[i].f]) {
            cmax(ans, (ind[i-1].s - ind[i-1].f + 1) + (ind[i].s - ind[i].f + 1) - 1);
        }
    }
    FOR(i, 1, sz(ind) - 1) {
        if(ind[i].f != ind[i].s) continue;
        if(ind[i+1].f == ind[i+1].s) continue;
        if(a[ind[i-1].s] < a[ind[i+1].f]) {
            cmax(ans, (ind[i-1].s - ind[i-1].f + 1) + (ind[i].s - ind[i].f + 1));
        }
    }
    put(ans);
