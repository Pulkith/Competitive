...
void solve() {
    ...
    if(lftcnt > rightcnt) {
        for(auto [x, y] : left) {
            int dif = lftcnt - rightcnt;
            if(dif == 0) break;
            int rh = (right.find(x) != right.end() ? right[x] : 0);
            if(y > 1 && y > rh) {
                int move = min((y - rh) / 2, dif / 2);
                cnt += move;
                right[x] += move;
                left[x] -= move;
                rightcnt += move; lftcnt -= move;
            }
        }
        
    } else if(rightcnt > lftcnt) {
        for(auto [x, y] : right) {
            int dif = rightcnt - lftcnt;
            if(dif == 0) break;
            int rh = (left.find(x) != left.end() ? left[x] : 0);
            if(y > 1 && y > rh) {
                int move = min((y-rh) / 2, dif / 2);
                cnt += move;
                right[x] -= move;
    ...
    for(auto [x,y] : left) {
        if(left[x] > 0 && right[x] > 0) {
            int dif = min(left[x], right[x]);
            left[x] -= dif;
            right[x] -= dif;
            rightcnt -= dif; lftcnt -= dif;
        }
    }
    if(rightcnt != lftcnt){
        cnt += (abs(lftcnt - rightcnt) / 2);
        int dif = (abs(lftcnt - rightcnt) / 2);
        if(rightcnt < lftcnt)
            rightcnt += dif, lftcnt -= dif;
        else
            lftcnt += dif, rightcnt -= dif;
    }
    cnt += (lftcnt);
