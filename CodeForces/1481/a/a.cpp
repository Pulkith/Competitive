...
inline namespace CP {
    ...
    inline namespace Output {
        ...
        string to_string(const char* s) { return string(s);}
            ...
            #define dbg(...)
        #endif
        template<class T> void outv(vector<T> v, bool standard = 1) {for(T& i : v) (standard?cout:cerr) << i << " "; cout << '\n'; }
...
void test_case([[maybe_unused]] int test_case = 0) {
    ...
    int ok = 1;
    if(y != 0) ok &= (count(str.begin(), str.end(), y < 0 ? 'D' : 'U') >= abs(y));
    if(x != 0) ok &= (count(str.begin(), str.end(), x < 0 ? 'L' : 'R') >= abs(x));
