...
int test_case(int test_case = 0) {
    cin >> N;
    string s; cin >> s;
    int count_left = count(s.begin(), s.end(), 'L');
    if(count_left == N || count_left == 0) {
        put(N < 3 ? 0 : cdiv(N, 3));
    } else {
        int same_ = 0;
        while(s[0] == s.back()) ++same_, s.pop_back();
        int i = 0;;
        while(s[i] == s[0]) ++same_, ++i;
        int ans = same_ / 3;
        for(; i < sz(s) - 2; ++i) {
            if(s[i] == s[i+1] && s[i+1] == s[i+2]) {
                char front = s[i]; int cnt = 0;
                while(front == s[i] && i < sz(s) )++i, ++cnt;
                ans += cnt / 3;
                --i;
            }
        }
