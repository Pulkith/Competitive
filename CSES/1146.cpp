...
//sum up number of on bits in binary representation of numbers from 1 to N
void test_case([[maybe_unused]] int test_case = 0) {
    ll N; cin >> N;
    int cnt = 0;
    while((1LL << (1LL * cnt)) <= N) ++cnt;
    ll ans = 0;
    FOR(i, 0, cnt) {
        ll seg = 1LL << (i);
        ll total = (N - seg + 1) / seg;
        ll dif_cnt = cdiv(total, 2);
        ll cur = dif_cnt * seg;
        if(total%2==0)
            cur += N - (seg)- (total * seg) + 1;
