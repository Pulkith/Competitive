...
int test_case([[maybe_unused]] int test_case = 0) {
    ...
    int ans = 0;
    set<int> cur;
    FOR(i, 0, N) ans += (cur.find(a[i]-1) == cur.end()), cur.insert(a[i]);

    auto calc = [&](int val, int index, int new_index) -> int {
        int res = 0;

        if(val != 1) {
            if(ind[val-1] < index && ind[val-1] > new_index) ++res;
            else if(ind[val-1] > index && ind[val-1] < new_index) --res;
        }
    ...
    while(Q--) {
        ...
        ans += calc(num1, x, y) + calc(num2, y, x);

        swap(a[x], a[y]);
        swap(ind[num1], ind[num2]);
