...
int N, M, K;
int apartments[200005];
int applications[200005];

void test_case([[maybe_unused]] int test_case = 0) {
    cin >> N >> M >> K;

    FOR(i, 0, N) cin >> applications[i];
    FOR(i, 0, M) cin >> apartments[i];
    
    sort(apartments, apartments+M);
    sort(applications, applications+N);

    int index_arp = 0;
    int ans = 0;
    FOR(i, 0, N) {
        while(index_arp < M && applications[i] > apartments[index_arp] + K) ++index_arp;
        if(index_arp >= M) break;
        if(applications[i] >= apartments[index_arp]-K && applications[i] <= apartments[index_arp] + K)
            ++ans, ++index_arp;
