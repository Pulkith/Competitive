...
void floodfill(vector<vector<char>> &a, vector<vector<bool>> &b, int i, int j) {
    a[i][j] = '.';
    b[i][j] = 1;
    ++area;
    ...
    FR(x, 4) {
        ...
        if(nx >= 0 && nx < N && ny >= 0 && ny < N && a[nx][ny] == '#')
            ...
            perim += (nx >= 0 && nx < N && ny >=0 && ny < N && b[nx][ny] ? 0 : 1);
    }
    ...
    FOR(i, 0, N) {
        FOR(j, 0, N) 
            if(a[i][j] == '#') {
                ...
                floodfill(a, vis, i, j);
                pii res = {area, perim};
                if((res.f == maxx.f && res.s < maxx.s) || res.f > maxx.f)
                    maxx = res;  
            }
    }
    cout << maxx.f << " " << maxx.s;
