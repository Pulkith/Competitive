...
struct pred { bool operator()(const vt<pr<int, int>> &l, const vt<pr<int, int>> &r) { 
    return l[0].f == r[0].f ? l[0].s < r[0].s : l[0].f < r[0].f;
} };

/*|||||||||||||||||| ||||||||||||||||||  CODE STARTS HERE  |||||||||||||||||| |||||||||||||||||| */

/* Check if 2 lines(line AB and line CD) intersect on a 2D grid */
bool intersect(pr<int, int> A, pr<int, int> B, pr<int, int> C, pr<int, int> D) {
    auto ccw = [&](pr<int, int> one, pr<int, int> second, pr<int, int> three) -> bool {
        return (three.s - one.s) * (second.f - one.f) > (second.s - one.s) * (three.f - one.f);
    };
...
void solve() {
    int N; cin >> N;
    priority_queue<vt<pr<int, int>>, vt<vt<pr<int, int>>>, pred> pq;
    FOR(i, 0, N) {
        int x1, y1, x2, y2;
