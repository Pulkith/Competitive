...
struct pred { bool operator()(const std::pair<int, int> &l, const std::pair<int, int> &r) { return (l.s == r.s ? l.f > r.f : l.s < r.s); } };
...
void solve() {
    ...
    priority_queue<pair<ll, ll>, vector<pair<ll, ll>>, pred> pq;
    ...
    ll ans = 0;
    while(!pq.empty() ) {
        if(B * pq.top().f - timeB < 0) {
            pq.pop(); continue;
        }
        auto top = pq.top();
        ll time = B * top.f - timeB;
        timeF += time;
        timeB += time;

        ll maxtime = (F * top.f - timeF);
        ans += maxtime * top.s;
        pq.pop();
        timeF += maxtime;
    }
